...
public class Player extends AppCompatActivity {
    ...
    ArrayList<SOSong> songArrayList;
    ArrayList<SOSong> temp;
    Random random;
    DiscMusic discMusic;
    ViewPager viewPager;
    int position = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (bundle == null) {
            ...
            String url = url_api + song.getPath();
            playMusic(url);
            for(int i = 0; i < songArrayList.size(); i++){
                if(Objects.equals(songArrayList.get(i).getId(), song.getId())){
                    position = i;
                }
            }
        }
        temp = (ArrayList<SOSong>) songArrayList.clone();
    ...
    private void btnPrevAction() throws InterruptedException {
        ...
        txtTitle.setText(songArrayList.get(position).getName());
        txtAuthorName.setText(songArrayList.get(position).getSinger().getName());
        String url = url_api + songArrayList.get(position).getPath();
        btnPlay.setImageResource(R.drawable.nutplay);
        playMusic(url);
    }
                        ...
                        if (!isSuffleClick) {
                            if (isRepeatClick == 2) {
                                ...
                                if (isRepeatClick == 3) {
                                    ...
                                    txtTitle.setText(songArrayList.get(position).getName());
                                    txtAuthorName.setText(songArrayList.get(position).getSinger().getName());
                                    playMusic(url_api + songArrayList.get(position).getPath());
                                } else {
                                    if (isRepeatClick == 1) {
                                        position++;
                                        if (position > songArrayList.size() - 1) {
                                            ...
                                            txtTitle.setText(songArrayList.get(position).getName());
                                            txtAuthorName.setText(songArrayList.get(position).getSinger().getName());
                                            initFirstSong(url_api + songArrayList.get(position).getPath());
                                            mediaPlayer.pause();
                                        } else {
                                            btnPlay.setImageResource(R.drawable.nutplay);
                                            if (mediaPlayer.isPlaying()) {
                            ...
                            if (isRepeatClick == 1) {
                                ...
                                if (!mediaPlayer.isPlaying()) {
                                    if (temp.size() > 0) {
                                        ...
                                        btnPlay.setImageResource(R.drawable.nutplay);
                                        txtAuthorName.setText(temp.get(position).getSinger().getName());
                                        initFirstSong(url_api + temp.get(position).getPath());
                                        playMusic(url_api + temp.get(position).getPath());
                                    } else {
                                        temp = (ArrayList<SOSong>) songArrayList.clone();
                                        position = random.nextInt(temp.size());
                                        btnPlay.setImageResource(R.drawable.nutpause);
                                        ...
                                        txtTitle.setText(temp.get(position).getName());
                                        txtAuthorName.setText(temp.get(position).getSinger().getName());
                                        initFirstSong(url_api + temp.get(position).getPath());
                                        mediaPlayer.pause();
                                    }
...
//        Set time
    ...
    private void starDownload() {
        pathDownload = url_api + song.getPath();
        ...
        request.setDescription("Downloading " + song.getName());
